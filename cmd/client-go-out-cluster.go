...
var clientgooutcluster = &cobra.Command{
	Use:   "client-go-out-cluster",
...
func init() {
	rootCmd.AddCommand(clientgooutcluster)
	clientgooutcluster.PersistentFlags().BoolVarP(&create, "create", "c", false, "Create the book-project ConfigMap, Deployment, and Service")
	clientgooutcluster.PersistentFlags().BoolVarP(&update, "update", "u", false, "Update the book-project Deployment (replicas or image)")
	clientgooutcluster.PersistentFlags().BoolVarP(&delete, "delete", "d", false, "Delete the book-project ConfigMap, Deployment, and Service")
	clientgooutcluster.PersistentFlags().IntVarP(&replicas, "replicas", "r", 1, "Number of replicas for the Deployment (used with --update)")
	clientgooutcluster.PersistentFlags().StringVarP(&image, "image", "i", "yourdockerhubusername/book-project:latest", "Docker image for the Deployment (used with --update)")
}
